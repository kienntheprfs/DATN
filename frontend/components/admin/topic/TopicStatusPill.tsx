...
export function TopicStatusPill({ status, pinned, knowledgeUpdated }: TopicStatusPillProps) {
  ...
  return (
    <>
      ...
      {knowledgeUpdated && (
        <span className="inline-flex items-center gap-1 rounded-full border border-pink-200 bg-pink-500/10 px-1.5 py-0.5 text-[11px] font-bold uppercase tracking-wider text-pink-600">
      ...
      {!pinned && !knowledgeUpdated && (
        <span
          className={`inline-flex items-center gap-1 rounded-full border px-1.5 py-0.5 text-[11px] font-bold uppercase tracking-wider ${cls}`}
        >
