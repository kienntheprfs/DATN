...
export const mapApi = {
  ...
  getCampus: async () => {
    const res = await apiClient.get<MapData[]>('/wayfinder/api/maps/campus');
    return res.data.map(m => ({ ...m, image_url: getFullImageUrl(m.image_url) }));
  },

  upload: async (formData: FormData) => {
    const res = await apiClient.post<MapData>('/wayfinder/api/maps', formData);
    return { ...res.data, image_url: getFullImageUrl(res.data.image_url) };
  },

  delete: async (id: number) => {
    await apiClient.delete(`/wayfinder/api/maps/${id}`);
  },

  updateMap: async (id: number, data: Partial<MapData>) => {
    const res = await apiClient.patch<MapData>(`/wayfinder/api/maps/${id}`, data);
    return { ...res.data, image_url: getFullImageUrl(res.data.image_url) };
  }
};
