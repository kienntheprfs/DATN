...
def get_pretrained_model(
    ...
    if repo_id in multi_lingual_models:
        return multi_lingual_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in chinese_models:
        return chinese_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in chinese_dialect_models:
        return chinese_dialect_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in english_models:
        return english_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in chinese_english_mixed_models:
        return chinese_english_mixed_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in chinese_cantonese_english_models:
        return chinese_cantonese_english_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in chinese_cantonese_english_japanese_korean_models:
        return chinese_cantonese_english_japanese_korean_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in cantonese_models:
        return cantonese_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in tibetan_models:
        return tibetan_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in arabic_models:
        return arabic_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in german_models:
        return german_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in french_models:
        return french_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in japanese_models:
        return japanese_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in russian_models:
        return russian_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in korean_models:
        return korean_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in thai_models:
        return thai_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in vietnamese_models:
        return vietnamese_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    elif repo_id in portuguese_brazlian_models:
        return portuguese_brazlian_models[repo_id](repo_id, decoding_method=decoding_method, num_active_paths=num_active_paths)
    else:
...
@lru_cache(maxsize=10)
def _get_offline_pre_trained_model(repo_id: str, decoding_method: str, num_active_paths: int) -> sherpa_onnx.OfflineRecognizer:
    assert repo_id in (
...
@lru_cache(maxsize=10)
def _get_vietnamese_pretrained_model(repo_id: str, decoding_method: str, num_active_paths: int) -> sherpa_onnx.OfflineRecognizer:
    # assert repo_id in (
...
@lru_cache(maxsize=10)
def _get_yifan_thai_pretrained_model(repo_id: str, decoding_method: str, num_active_paths: int) -> sherpa_onnx.OfflineRecognizer:
    assert repo_id in ("yfyeung/icefall-asr-gigaspeech2-th-zipformer-2024-06-20",), repo_id
...
@lru_cache(maxsize=10)
def _get_zrjin_cantonese_pre_trained_model(repo_id: str, decoding_method: str, num_active_paths: int) -> sherpa_onnx.OfflineRecognizer:
...
@lru_cache(maxsize=10)
def _get_russian_pre_trained_model_ctc(repo_id: str, decoding_method: str, num_active_paths: int) -> sherpa_onnx.OfflineRecognizer:
    assert repo_id in (
...
@lru_cache(maxsize=10)
def _get_russian_pre_trained_model(repo_id: str, decoding_method: str, num_active_paths: int) -> sherpa_onnx.OfflineRecognizer:
    assert repo_id in (
...
@lru_cache(maxsize=10)
def _get_moonshine_model(repo_id: str, decoding_method: str, num_active_paths: int) -> sherpa_onnx.OfflineRecognizer:
...
@lru_cache(maxsize=10)
def _get_whisper_model(repo_id: str, decoding_method: str, num_active_paths: int) -> sherpa_onnx.OfflineRecognizer:
    ...
    tokens = _get_token_filename(repo_id=full_repo_id, subfolder=".", filename=f"{name}-tokens.txt")
...
def _get_english_model(
    ...
    if repo_id == "WeijiZhuang/icefall-asr-librispeech-pruned-transducer-stateless8-2022-12-02":
        filename = "cpu_jit-torch-1.10.pt"

    if repo_id == "yfyeung/icefall-asr-multidataset-pruned_transducer_stateless7-2023-05-04":
        filename = "cpu_jit-epoch-30-avg-4.pt"

    if repo_id == "yfyeung/icefall-asr-finetune-mux-pruned_transducer_stateless7-2023-05-19":
...
def _get_fire_red_asr_models(repo_id: str, decoding_method: str, num_active_paths: int):
    assert repo_id in ("csukuangfj/sherpa-onnx-fire-red-asr-large-zh_en-2025-02-16",), repo_id
...
def _get_tibetan_pre_trained_model(
    ...
    filename = "cpu_jit.pt"
    if repo_id == "syzym/icefall-asr-xbmu-amdo31-pruned-transducer-stateless5-2022-11-29":
...
def _get_sherpa_onnx_offline_zipformer_pre_trained_model(
    ...
    if repo_id == "csukuangfj/sherpa-onnx-zipformer-en-libriheavy-20230926-large":
        ...
        epoch = 90
        avg = 20
    elif repo_id == "csukuangfj/sherpa-onnx-zipformer-en-libriheavy-20230830-large-punct-case":
        epoch = 16
        avg = 2
    elif repo_id == "csukuangfj/sherpa-onnx-zipformer-en-libriheavy-20230830-medium-punct-case":
        epoch = 50
        avg = 15
    elif repo_id == "csukuangfj/sherpa-onnx-zipformer-en-libriheavy-20230830-small-punct-case":
        epoch = 88
        avg = 41
...
def _get_japanese_pre_trained_model(
    ...
    assert repo_id in ["TeoWenShen/icefall-asr-csj-pruned-transducer-stateless7-streaming-230208"], repo_id
    assert kind in ("fluent", "disfluent"), kind

    encoder_model = _get_nn_model_filename(repo_id=repo_id, filename="encoder_jit_trace.pt", subfolder=f"exp_{kind}")

    decoder_model = _get_nn_model_filename(repo_id=repo_id, filename="decoder_jit_trace.pt", subfolder=f"exp_{kind}")

    joiner_model = _get_nn_model_filename(repo_id=repo_id, filename="joiner_jit_trace.pt", subfolder=f"exp_{kind}")
...
def _get_paraformer_en_pre_trained_model(
    ...
    tokens = _get_token_filename(repo_id=repo_id, filename="new_tokens.txt", subfolder=".")
...
@lru_cache(maxsize=5)
def _get_chinese_dialect_models(repo_id: str, decoding_method: str, num_active_paths: int) -> sherpa_onnx.OfflineRecognizer:
    assert repo_id in [
